function fives = findingfives(matrix)
    ...
    % fives, from the highest and most leftmost point have a right branch
        % for font 72 calibri we move 33 pixels right initially
    ...
    % iterating over every pixel in the matrix and checking if it's a black pixel and if so implementing our logic to test if it is an eight.
    for row = 1:rows
        for column = 1:columns
            if(matrix(row, column)==0)
            
                % ininitialising our variables
                test = false;  % sentinel variable to control the while loop
                xcrement_var = 0; % variable to control the incrementation / pseudomovement of the vector, as we will change the x (column) and y (row) components for each iteration of the loop and test if the pixel currently 
                % on is still black
                ...
                while ~test
                    if(stage==1) % moving 30 columns to the right as this should take us to the end of the top branch of the 5. If this passes 
                        ...
                        if(matrix(row, column+xcrement_var)==0 && xcrement_var < 30) % if the current pixel is still black and the length of the vector is less than the required length
                            xcrement_var =  xcrement_var + 1; % continuing to move in the direction required
                        elseif(xcrement_var == 30) % if we have reached the end of the required vector we have passed this stage and can hence move on to the next stage
                            stage = stage + 1;
                            xcrement_var = 0;
                        else % if a pixel in this final vector was not black, we have not passed the vector test and hence need to return to the main function with this info
                            fprintf("Not a five\n");
                    ...
                    elseif(stage==2) % moving 33 down to start moving right to reach the curve
                        if(matrix(row+xcrement_var, column)==0 && xcrement_var < 33) % if the current pixel is still black and the length of the vector is less than the required length
                            xcrement_var =  xcrement_var + 1;
                        elseif(xcrement_var == 33) % if we have reached the end of the required vector we have passed this stage and can hence move on to the next stage
                            stage = stage + 1;
                    ...
                    elseif(stage == 3) % moving right so that we can start the curve of the 5, if all of the pixels while moving right are
                        ...
                        if(matrix(new_row, new_column+xcrement_var)==0 && xcrement_var < 23) % if the current pixel is still black and the length of the vector is less than the required length
                            xcrement_var =  xcrement_var + 1;
                    ...
                    elseif(stage == 4) % moving outwards to reach the rightmost point of the curve of the five
                        if(matrix(new_row + xcrement_var, new_column+xcrement_var)==0 && xcrement_var < 16) % if the current pixel is still black and the length of the vector is less than the required length
                            xcrement_var =  xcrement_var + 1;
                    ...
                    elseif(stage == 5) % moving inwards to move to the bottom of the 5
                        if(matrix(new_row - xcrement_var, new_column-xcrement_var)==0 && xcrement_var < 18) % if the current pixel is still black and the length of the vector is less than the required length
                            xcrement_var =  xcrement_var + 1;
                    ...
                    elseif(stage == 6) % checking the tail end of the shape to ensure that it matches with that of a 5, and if so
                        if(matrix(new_row, new_column-xcrement_var)==0 && xcrement_var < 27) % if the current pixel is still black and the length of the vector is less than the required length
                            xcrement_var =  xcrement_var + 1;
                        elseif(xcrement_var == 27) % if we have reached the required length for the final vector we can finish the while loop and return that we have found a five
                            test = true;
                        else % if a pixel in this final vector was not black, we have not passed the vector test and hence need to return to the main function with this info
                            fprintf("Not a five\n");
%                             fprintf("1");
%                             fprintf("%d, %d", row, column+xcrement_var);
                        ...
                        test = true; % ending the while loop so that we can close the function
                    end
                end
