...
%                         fprintf("1");
                ...
                for i = 1:11
                    if(matrix(new_row - i, new_column - i)~=0)% if the pixel is no longer black we return to the main function as this vector has not passed the test
                        ...
                        return;
                    elseif(i==11)  % if all pixels iterated over were black and we are now at the 9th pixel, we can return the new_row and column to the 
                        % next vector, so that we can start iterating from there
                        new_row = new_row-i;
                        new_column = new_column - i;
                    end
                ...
                % as all the tests passed, we have a 3.
    ...
    threes = 0; % returning zero threes if all of the pixels are iterated through and none are black
    % hence there are no numbers in the grid
end
