        ...
        for column = 1:columns
            if(matrix(row, column)==0)
                ...
                if(stage==1)  % moving right along the top right branch of the 5
                    if(findingnumbers("right", 26, row, column, matrix)~=true) % if all of the pixels iterated over are not black
                        fprintf("Not a 5\n");
                        fives = 0;
                        return;
                ...
                elseif(stage == 6) % moving back left to finish the tail of the 5
                    if(findingnumbers("left", 22, new_row, new_column, matrix)~=true) % if all the pixels iterated over are not black we return false
                        fprintf("Not a 5\n");
                        fives = 0;
                        return;
                    else
