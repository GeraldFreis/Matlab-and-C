            ...
            if(matrix(row, column)==0) % if the pixel is black applying our logic
                ...
                while ~test
                    ...
                    elseif(stage==7) % moving 11 columns down and left                    
                        if(findingnumbers("downleft", 11, new_row, column_right+3, matrix)==false) % if not all of the pixels iterated over by the vector are black
                            fprintf("Not a 9\n");
                            nines = 0;
                            return;
                        else
                            new_row = new_row + 12; % updating the new_row_left so that this row can be used for the next vector
