function sixes = findingsixes(matrix)
    [rows, columns] = size(matrix); % finding the rows and columns of the matrix so that we can iterate through every pixel
    % logic for six:
    ...
    %% iterating over every pixel in the matrix and if the pixel is black applying our logic to test if it is connected to a 6
    for row = 1:rows
