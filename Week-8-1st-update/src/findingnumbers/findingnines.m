function nines = findingnines(matrix, numberofdigits)
    [rows, columns] = size(matrix); % finding the number of rows and columns in the matrix
    
    % comment
        % as with eights, there are adjustments of the vectors in 9 between 1 digit AO's and 2 digit AO's
        % and so we need to initialise adjustment_factor variables at certain times throughout the code,
        % that influence the vector lengths and columns and rows of the vectors.
                ...
                while ~test
                    ...
                    if(stage == 1) % iterating down and outwards around the top circle of the nine, and checking to ensure that every pixel iterated over is black
                        ...
                        elseif(xcrement_var == 5) % if every pixel iterated over was black and we have reached the length of our vector
                            ...
                            new_row = new_row + xcrement_var-4; % updating the row so that our next vector picks up from this row with each column variable to form the next variable
                            new_row_left = new_row; % updating the row for the left vector, and ensuring that it is in the centre of the lines for the 9, as the 9 is not proportional
                    ...
                    elseif(stage==2) % iterating 10 pixels outwards and downards from the terminal points of the last vector
                        ...
                        elseif(xcrement_var == 10) % if every pixel iterated over was black and we have reached the required length of the vector
                            ...
                            stage = stage + 1; % increasing the stage as we have passed this vec tor test
                            xcrement_var = 0;
                    ...
                    elseif(stage==6) % moving 17 rows down with our right vector
                        if(findingnumbers("down", 17, new_row, column_right, matrix)==false) % if not all of the pixels iterated over by the vector are black
                            ...
                            new_row = new_row + 18; % updating the new_row so that this row can be used as the initial row for the next vector
                            stage = 7;
                        end
                    
                     % in the next vector there is an adjustment between 1 digit nines and 2 digit nines
                     % that arises. hence we need variables to control which vector is used in which
                     % circumstance

                     %% initialising adjustment factors
                     if(numberofdigits == 2)
                         adjustment_column = 3;
                         adjustment_updated_row = 1;
                     elseif(numberofdigits == 1)
                         adjustment_column = 0;
                         adjustment_updated_row = 2;
                     else
                         fprintf("We do not support that number of digits")
                     end

                    elseif(stage==7) % moving 11 columns down and left                    
                        if(findingnumbers("downleft", 11, new_row, column_right+adjustment_column, matrix)==false) % if not all of the pixels iterated over by the vector are black
                            fprintf("Not a 9\n");
                            nines = 0;
                            return;
                        else % if all of the pixels itearted over were black:
                            new_row = new_row + 11 + adjustment_updated_row; % updating the new_row_left so that this row can be used for the next vector
                    ...
                    elseif(stage==8) % moving 18 columns left                        
                        if(findingnumbers("left", 18, new_row, column_right, matrix)==false) % if not all of the pixels iterated over were black
                            fprintf("Not a 9\n");
                            nines = 0;
                            fprintf("%d, %d", new_row, column_right);
                            return;
                        else % if all of the pixels iterated over were black we can increase the stage so that we can return that a nine was found
                            stage = 9;
                        end

                    else % as all tests have been passed, we have found a nine and need to return this information
                        nines = 1;
