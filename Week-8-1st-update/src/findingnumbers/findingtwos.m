    ...
    for row = 1:rows
        for column = 1:columns
            ...
            if(matrix(row, column)==0)
                ...
                %% using a while loop to iterate through the stages
                while ~test
                    ...
                    elseif(stage==2) % moving 4 pixels down and left and checking if all of the pixels iterated over are black, and if so we have passed this test
                        ...
                        if(findingnumbers("downleft", 3, new_row, new_column, matrix)==false) % if not every pixel iterated over in the vector was black
                            fprintf("Not a 2\n");
                            twos = 0;
                            return;
                    ...
                    elseif(stage==3) % moving right and down to follow the right shape of the 2
                        if(findingnumbers("downright", 5, row, column, matrix)==false) % if not all of the pixels iterated over by the vector were black
                            fprintf("Not a 2\n");
                            twos = 0;
                            return;
                            ...
                            new_column = column + 5; % using the terminal column of this vector as the initial column for the next vector
                    ...
                    elseif(stage==4) % continuing to move down the right side of the 2, but moving until the rightmost edge of the circular part of the two                      
                        if(findingnumbers("downright", 12, new_row, new_column, matrix)==false) % if not all of the pixels iterated over were black
                            ...
                            new_row = new_row + 20; % using the terminal point of this vector + 5 rows as the initial point for the next vector
                    ...
                    elseif(stage==5) % moving inwards and thus increasing the rows and decreasing the columns
                        ...
                        if(findingnumbers("downleft", 9, new_row, new_column, matrix)==false) % if not all of the pixels iterated over were black
                            ...
                            new_row = new_row + 13; % using the terminal point of this vector as the initial point for the next vector
                            new_column = new_column - 9;
                            stage = 6;
                        end

                    elseif(stage==6) % moving inwards 20 pixels (20 rows down and 20 columns left)
                    ...
                    else % if every stage was passed over but the code was kicked out
                        fprintf("Not a 2\n");
                        twos = 0;
                        return;
                    end
